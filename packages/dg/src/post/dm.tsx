...
import { AppStore, Close, SimpleDialog, UserBasic, elipsis, personIcon, useApp } from "../theme"
import { Query , QueryResult, SimpleQueryBody, VirtualItem, newQuery} from "../query"
...
export function userList(g: Group) {
    return g.user.map((u)=> u.name).join(', ')
}
export function handleList(g: Group) {
    return g.user.map((u)=> '@'+u.handle).join(', ')
}
